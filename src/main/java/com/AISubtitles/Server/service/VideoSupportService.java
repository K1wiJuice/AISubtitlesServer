...
public class VideoSupportService {
    ...
    public  Result compressVideo( int videoId, int b) {
        ...
        String videoPath = "/home/ubuntu" + video.getVideoPath();
        String compressedVideoPath = "/home/ubuntu/video/" + video.getIdentifier() + "/" + video.getIdentifier()+"-compress.mp4";
        try {
    ...
    public  Boolean exportAudio(int videoId) {
        video = videoDao.findById(videoId).get();
        String videoPath = "/home/ubuntu" + video.getVideoPath();
        String audioPath ="/home/ubuntu/audio/"+videoId+".mp3";
   ...
   //  * @param videoPath             视频路径
    ...
    public  Result importSubtitle(int videoId , String type) {
        ...
        String videoPath = "/home/ubuntu" + video.getVideoPath();
        String subtitlePath = "";
        if(type.equals("zh")){ subtitlePath = subtitle.getZhSubtitlePath();}
        else if (type.equals("en")){ subtitlePath = subtitle.getEnSubtitlePath();}
        else { subtitlePath = subtitle.getMergeSubtitlePath();}
        //String subtitlePath = "C:/Users/11392/Desktop/test/01/subtitles.srt";
        String videoWithSubtitlePath = "/home/ubuntu/video/" + video.getIdentifier() + "/"+video.getIdentifier() +"-" + type + ".mp4";

        final List<String> commList = new ArrayList<>(
            Arrays.asList("ffmpeg", "-i", videoPath, "-vf", "subtitles=" + subtitlePath, "-y", videoWithSubtitlePath));
    ...
    public void voiceChanger(int videoId, final int audioType) {
        ...
        if(exportAudio(videoId)){
         voicePath = "/home/ubuntu/audio/"+videoId+".mp3";
         outputPath = "/home/ubuntu/audio/"+videoId+"output.mp3";}
        SoundEnum soundEnum;
        switch (audioType) {
            case 1:
    ...
    public Result replaceAudio(int videoId,int audioType) {
        ...
        video = videoDao.findById(videoId).get();
        String videoPath = "/home/ubuntu" + video.getVideoPath();
        String audioPath = "/home/ubuntu/audio/"+videoId+"output.mp3";

        String outputPath = "/home/ubuntu/video/"+video.getIdentifier()+"/"+video.getIdentifier()+"-ra.mp4";
        try {

            final List<String> commList = new ArrayList<>(
                Arrays.asList("ffmpeg", "-i", videoPath, "-i", audioPath, "-c:v", "copy", "-c:a", "aac", "-strict", "experimental", "-map", "0:v:0", "-map", "1:a:0", "-y", outputPath));
            ExecuteCommandService.exec(commList);

            video.setVideoPath(outputPath);

            File dir = new File("/home/ubuntu/audio/");
