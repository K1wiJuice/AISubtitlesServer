...
public class ChunkServiceImpl implements ChunkService {
    ...
    //没看明白。。。。。。
    @Override
    public boolean checkChunk(String videoNameAndUserId, Integer chunkNumber) {
        Specification<Chunk> specification = (Specification<Chunk>) (root, criteriaQuery, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            predicates.add(criteriaBuilder.equal(root.get("videoNameAndUserId"), videoNameAndUserId));
